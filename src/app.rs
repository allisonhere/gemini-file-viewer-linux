...
impl FileViewerApp {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        // Set slightly larger, cleaner defaults before loading settings
        {
            let ctx = &cc.egui_ctx;
            let mut style = (*ctx.style()).clone();
            // Global text sizing
            use egui::TextStyle as TS;
            style.text_styles.insert(TS::Body, egui::FontId::new(15.0, egui::FontFamily::Proportional));
            style.text_styles.insert(TS::Button, egui::FontId::new(15.0, egui::FontFamily::Proportional));
            style.text_styles.insert(TS::Heading, egui::FontId::new(22.0, egui::FontFamily::Proportional));
            style.text_styles.insert(TS::Monospace, egui::FontId::new(14.0, egui::FontFamily::Monospace));
            style.spacing.item_spacing = egui::vec2(10.0, 8.0);
            style.spacing.button_padding = egui::vec2(12.0, 8.0);
            ctx.set_style(style);
        }
        // Try to load bundled fonts if present.
    ...
    pub(crate) fn apply_theme(&self, ctx: &egui::Context) {
        ...
        // Slightly softer strokes
        visuals.widgets.inactive.fg_stroke = egui::Stroke::new(1.0, visuals.text_color());
                    ...
                    egui::Frame::new()
                        ...
                        .show(ui, |ui| {
                            ui.vertical_centered(|ui| {
                                ...
                                ui.label(RichText::new("🗂️").size(42.0));
                                ui.add_space(4.0);
                                ui.label(RichText::new("Gemini File Viewer").heading().color(accent));
                                ui.label("Open a file to get started.");
                                ui.add_space(8.0);
                                if ui.button(RichText::new("📂 Open File").strong()).clicked() {
                                    if let Some(path) = rfd::FileDialog::new()
                                        .add_filter("All Supported", &["txt","rs","py","toml","md","json","js","html","css","png","jpg","jpeg","gif","bmp","webp"])
                                        .add_filter("Images", &["png","jpg","jpeg","gif","bmp","webp"])
                                        .add_filter("Text/Source", &["txt","rs","py","toml","md","json","js","html","css"])
                                        .pick_file() {
                                        file_to_load.replace(path);
                                    }
                                }
                            });
                        });
